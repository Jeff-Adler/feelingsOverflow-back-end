...
for i in 0...20 do
    ...
    Comment.create!(comment: Faker::Lorem.sentence(word_count: 3), votes:rand(1..5), post: post, commenter_id: user.id)
    Comment.create!(comment: Faker::Lorem.sentence(word_count: 3), votes:rand(1..5), post: post, commenter_id: user.id)
end

Post.all.each do |post|
    post.comments.create(comment: Faker::Lorem.sentence(word_count: 3), votes:rand(1..5), commenter_id: User.all.first.id)
    post.comments.create(comment: Faker::Lorem.sentence(word_count: 3), votes:rand(1..5), commenter_id: User.all.first.id)
end
